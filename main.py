# Comprehensive Statistical Analysis of Dormitory Survey Data
# President University Student Dormitory Survey
...
print("\nDescriptive Statistics for Satisfaction Variables:")
...
for col in numeric_cols:
    ...
    print(f"Mean (μ) = Σx/n = ({df[col].sum()}/{len(df)}) = {mean:.4f}")
    print(f"Median = {median}")
    print(f"Mode = {mode}")
    print(f"Variance (σ²) = Σ(x-μ)²/n = {variance:.4f}")
    print(f"Standard Deviation (σ) = √σ² = {std_dev:.4f}")
    print(f"Coefficient of Variation (CV) = σ/μ × 100% = {cv:.2f}%")
    print(f"Skewness = {skewness:.4f}")
    print(f"Kurtosis = {kurtosis:.4f}")
    print(f"Range = {df[col].max() - df[col].min()}")
    print(f"Interquartile Range (IQR) = {df[col].quantile(0.75) - df[col].quantile(0.25)}")
    
    # Calculate a step-by-step example for one data point's contribution to variance
    example_value = df[col].iloc[0]
    contribution_to_variance = (example_value - mean)**2
    print(f"\nStep-by-step variance calculation example for first data point:")
    print(f"Value = {example_value}")
    print(f"Deviation from mean = (x - μ) = ({example_value} - {mean:.4f}) = {example_value - mean:.4f}")
    print(f"Squared deviation = (x - μ)² = {contribution_to_variance:.4f}")
    print(f"This value's contribution to total variance = {contribution_to_variance/len(df):.4f}")
...
for col in categorical_cols:
    ...
    for category, count in counts.items():
        ...
        print(f"{category}: {count} respondents ({percentage}%)")
        print(f"  P({category}) = {count}/{len(df)} = {percentage}%")
